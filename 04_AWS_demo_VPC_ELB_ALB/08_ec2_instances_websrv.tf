...
resource "aws_vpc_security_group_ingress_rule" "demo04_sg_websrv_ingress_http_0" {
  security_group_id = aws_security_group.demo04_sg_websrv.id
  description       = "allow HTTP access from VPC"
  from_port         = 80
  to_port           = 80
  ip_protocol       = "tcp"
  cidr_ipv4         = var.cidr_vpc
  tags              = { Name = "demo04_sg_websrv-sgr-ingress-http-0" }
}

resource "aws_vpc_security_group_ingress_rule" "demo04_sg_websrv_ingress_ssh_1" {
  security_group_id            = aws_security_group.demo04_sg_websrv.id
  description                  = "allow SSH access from bastion host"
  from_port                    = 22
  to_port                      = 22
  ip_protocol                  = "tcp"
  referenced_security_group_id = aws_security_group.demo04_sg_bastion.id
  tags                         = { Name = "demo04_sg_websrv-sgr-ingress-ssh-1" }
}

resource "aws_vpc_security_group_egress_rule" "demo04_sg_websrv_egress_all_2" {
  security_group_id = aws_security_group.demo04_sg_websrv.id
  ...
  ip_protocol       = "-1"
  cidr_ipv4         = "0.0.0.0/0"
  tags              = { Name = "demo04_sg_websrv-sgr-egress-all-2" }
}
