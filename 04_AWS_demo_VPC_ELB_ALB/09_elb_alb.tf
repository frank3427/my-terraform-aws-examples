...
resource "aws_vpc_security_group_ingress_rule" "demo04_sg_alb_ingress_http_0" {
  count             = length(var.authorized_ips)
  security_group_id = aws_security_group.demo04_sg_alb.id
  description       = "allow HTTP access from authorized_ips"
  from_port         = 80
  to_port           = 80
  ip_protocol       = "tcp"
  cidr_ipv4         = var.authorized_ips[count.index]
  tags              = { Name = "demo04_sg_alb-sgr-ingress-http-0" }
}

resource "aws_vpc_security_group_egress_rule" "demo04_sg_alb_egress_http_1" {
  security_group_id            = aws_security_group.demo04_sg_alb.id
  description                  = "allow only HTTP traffic to web servers"
  from_port                    = 80
  to_port                      = 80
  ip_protocol                  = "tcp"
  referenced_security_group_id = aws_security_group.demo04_sg_websrv.id
  tags                         = { Name = "demo04_sg_alb-sgr-egress-http-1" }
}
