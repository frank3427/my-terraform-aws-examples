...
resource "aws_vpc_security_group_ingress_rule" "demo15b_acct1_pvd_sg_websrv_ingress_http_0" {
  count             = length(var.authorized_ips)
  security_group_id = aws_security_group.demo15b_acct1_pvd_sg_websrv.id
  description       = "allow HTTP access from authorized public IP addresses (thru NLB)"
  from_port         = 80
  to_port           = 80
  ip_protocol       = "tcp"
  cidr_ipv4         = var.authorized_ips[count.index]
  tags              = { Name = "demo15b_acct1_pvd_sg_websrv-sgr-ingress-http-0" }
}

resource "aws_vpc_security_group_ingress_rule" "demo15b_acct1_pvd_sg_websrv_ingress_http_health_1" {
  security_group_id = aws_security_group.demo15b_acct1_pvd_sg_websrv.id
  description       = "allow HTTP access from VPC public subnet (needed for health checks)"
  from_port         = 80
  to_port           = 80
  ip_protocol       = "tcp"
  cidr_ipv4         = var.acct1_pvd_cidr_subnet_public
  tags              = { Name = "demo15b_acct1_pvd_sg_websrv-sgr-ingress-http_health-1" }
}

resource "aws_vpc_security_group_ingress_rule" "demo15b_acct1_pvd_sg_websrv_ingress_ssh_2" {
  security_group_id = aws_security_group.demo15b_acct1_pvd_sg_websrv.id
  ...
  ip_protocol       = "tcp"
  cidr_ipv4         = var.acct1_pvd_cidr_subnet_public
  tags              = { Name = "demo15b_acct1_pvd_sg_websrv-sgr-ingress-ssh-2" }
}

resource "aws_vpc_security_group_egress_rule" "demo15b_acct1_pvd_sg_websrv_egress_all_3" {
  security_group_id = aws_security_group.demo15b_acct1_pvd_sg_websrv.id
  ...
  ip_protocol       = "-1"
  cidr_ipv4         = "0.0.0.0/0"
  tags              = { Name = "demo15b_acct1_pvd_sg_websrv-sgr-egress-all-3" }
}
