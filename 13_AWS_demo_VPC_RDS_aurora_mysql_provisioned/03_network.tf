# ------ Create a VPC 
resource aws_vpc demo13 {
  cidr_block           = var.cidr_vpc
  enable_dns_hostnames = true
  tags                 = { Name = "demo13-vpc" }
}

# ------ Create an internet gateway in the new VPC
resource aws_internet_gateway demo13 {
  vpc_id = aws_vpc.demo13.id
  tags   = { Name = "demo13-igw" }
}

# ------ Add a name and route rule to the default route table
resource aws_default_route_table demo13 {
  default_route_table_id = aws_vpc.demo13.default_route_table_id
  tags                   = { Name = "demo13-rt" }

  route {
    cidr_block = "0.0.0.0/0"
    gateway_id = aws_internet_gateway.demo13.id
  }
}

# ------ Add a name to the default network ACL and modify ingress rules
resource aws_default_network_acl demo13 {
  default_network_acl_id = aws_vpc.demo13.default_network_acl_id
  tags                   = { Name = "demo13-acl" }
