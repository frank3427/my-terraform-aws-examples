...
resource aws_eip demo14b_inst1 {
  instance = aws_instance.demo14b_inst1.id
  domain   = "vpc"
  tags     = { Name = "demo14b-inst1" }
}

# ------ Create an EC2 instance
resource aws_instance demo14b_inst1 {
...
ssh -i ${var.private_sshkey_path} ${local.username}@${aws_eip.demo14b_inst1.public_ip}

---- Alternatively, you can add the following lines to your file $HOME/.ssh/config and then just run "ssh d14"

Host d01
        Hostname ${aws_eip.demo14b_inst1.public_ip}
        User ${local.username}
