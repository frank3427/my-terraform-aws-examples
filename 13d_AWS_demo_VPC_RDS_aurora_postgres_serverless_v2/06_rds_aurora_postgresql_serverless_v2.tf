...
resource "aws_vpc_security_group_ingress_rule" "demo13d_rds_ingress_all_0" {
  security_group_id = aws_security_group.demo13d_rds.id
  description       = "allow PostgreSQL access from VPC"
  from_port         = 5432
  to_port           = 5432
  ip_protocol       = "tcp"
  cidr_ipv4         = var.cidr_vpc
  tags              = { Name = "demo13d_rds-sgr-ingress-all-0" }
}

resource "aws_vpc_security_group_egress_rule" "demo13d_rds_egress_all_1" {
  security_group_id = aws_security_group.demo13d_rds.id
  description       = "allow all traffic"
  ip_protocol       = "-1"
  cidr_ipv4         = "0.0.0.0/0"
  tags              = { Name = "demo13d_rds-sgr-egress-all-1" }
}
