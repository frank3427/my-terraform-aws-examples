...
resource "aws_vpc_security_group_ingress_rule" "demo41_sg_alb_ingress_http_0" {
  security_group_id = aws_security_group.demo41_sg_alb.id
  description       = "Allow HTTP from VPC"
  from_port         = 80
  to_port           = 80
  ip_protocol       = "tcp"
  cidr_ipv4         = "0.0.0.0/0"
  tags              = { Name = "demo41_sg_alb-sgr-ingress-http-0" }
}

resource "aws_vpc_security_group_egress_rule" "demo41_sg_alb_egress_all_1" {
  security_group_id = aws_security_group.demo41_sg_alb.id
  ...
  ip_protocol       = "-1"
  cidr_ipv4         = "0.0.0.0/0"
  tags              = { Name = "demo41_sg_alb-sgr-egress-all-1" }
}
