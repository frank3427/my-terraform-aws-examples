...
---- You can SSH directly to the Linux instance with MySQL client by typing the following ssh command
ssh -i ${var.private_sshkey_path} ${local.username}@${aws_eip.demo13c_db_client.public_ip}

---- Once connected, you can connect to the MySQL database using the following command
export MYSQL_PASSWD=${random_string.demo13c-db-passwd.result}
./mysql.sh
...
# ---- MySQL details
# Endpoint: ${aws_db_instance.demo13c_mysql.endpoint}
# User    : ${aws_db_instance.demo13c_mysql.username}
# Password: ${random_string.demo13c-db-passwd.result}