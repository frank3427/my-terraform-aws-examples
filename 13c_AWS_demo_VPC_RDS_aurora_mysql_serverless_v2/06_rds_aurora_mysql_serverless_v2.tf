...
resource "aws_vpc_security_group_ingress_rule" "demo13c_rds_ingress_all_0" {
  security_group_id = aws_security_group.demo13c_rds.id
  description       = "allow MySQL access from VPC"
  from_port         = 3306
  to_port           = 3306
  ip_protocol       = "tcp"
  cidr_ipv4         = var.cidr_vpc
  tags              = { Name = "demo13c_rds-sgr-ingress-all-0" }
}

resource "aws_vpc_security_group_egress_rule" "demo13c_rds_egress_all_1" {
  security_group_id = aws_security_group.demo13c_rds.id
  description       = "allow all traffic"
  ip_protocol       = "-1"
  cidr_ipv4         = "0.0.0.0/0"
  tags              = { Name = "demo13c_rds-sgr-egress-all-1" }
}
