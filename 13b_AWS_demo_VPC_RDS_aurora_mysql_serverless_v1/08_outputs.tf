...
ssh -i ${var.private_sshkey_path} ${local.username}@${aws_eip.demo13b_db_client.public_ip}

---- Once connected, you can connect to the MySQL database using the following command
export MYSQL_PASSWD=${random_string.demo13b-db-passwd.result}
./mysql.sh
...
# User    : ${aws_db_instance.demo13b_mysql.username}
# Password: ${random_string.demo13b-db-passwd.result}