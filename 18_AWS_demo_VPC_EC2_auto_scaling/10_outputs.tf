...
output CONNECTIONS {
  ...
  1) ---- SSH connection to compute instances
     ...
     ssh -F sshcfg d18-bastion             to connect to bastion host
     ssh -F sshcfg d18-ws1                 to connect to Web server #1
     ssh -F sshcfg d18-ws2                 to connect to Web server #2

  2) ---- HTTP connection to public load balancer
     Open the following URL in your Web browser:
     http://${aws_lb.demo18_alb.dns_name}
EOF
