...
resource "aws_vpc_security_group_ingress_rule" "demo23_ingress_http_0" {
  security_group_id = aws_default_security_group.demo23.id
  description       = "allow HTTP access from authorized public IP addresses"
  ip_protocol       = "-1"
  cidr_ipv4         = "0.0.0.0/0"
  tags              = { Name = "demo23-sgr-ingress-http-0" }
}
...
resource "aws_vpc_security_group_ingress_rule" "demo23_sg2_ingress_http_0" {
  ...
  tags              = { Name = "demo23_sg2-sgr-ingress-http-0" }
}
