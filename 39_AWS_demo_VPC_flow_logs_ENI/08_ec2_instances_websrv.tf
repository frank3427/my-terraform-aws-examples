...
resource "aws_vpc_security_group_ingress_rule" "demo39_sg_websrv_ingress_http_0" {
  ...
  tags              = { Name = "demo39_sg_websrv-sgr-ingress-http-0" }
}

resource "aws_vpc_security_group_ingress_rule" "demo39_sg_websrv_ingress_http_health_1" {
  ...
  tags              = { Name = "demo39_sg_websrv-sgr-ingress-http_health-1" }
}

resource "aws_vpc_security_group_ingress_rule" "demo39_sg_websrv_ingress_ssh_2" {
  count             = length(var.cidr_subnet_public)
  security_group_id = aws_security_group.demo39_sg_websrv.id
  ...
  ip_protocol       = "tcp"
  cidr_ipv4         = var.cidr_subnet_public[count.index]
  tags              = { Name = "demo39_sg_websrv-sgr-ingress-ssh-2" }
}
