...
resource "aws_vpc_security_group_ingress_rule" "demo12b_rds_ingress_all_0" {
  security_group_id = aws_security_group.demo12b_rds.id
  description       = "allow PostgreSQL access from VPC"
  from_port         = 5432
  to_port           = 5432
  ip_protocol       = "tcp"
  cidr_ipv4         = var.cidr_vpc
  tags              = { Name = "demo12b_rds-sgr-ingress-all-0" }
}

resource "aws_vpc_security_group_ingress_rule" "demo12b_rds_ingress_all_1" {
  security_group_id = aws_security_group.demo12b_rds.id
  ...
  ip_protocol       = "-1"
  cidr_ipv4         = var.cidr_vpc
  tags              = { Name = "demo12b_rds-sgr-ingress-all-1" }
}

resource "aws_vpc_security_group_egress_rule" "demo12b_rds_egress_all_2" {
  security_group_id = aws_security_group.demo12b_rds.id
  ...
  ip_protocol       = "-1"
  cidr_ipv4         = "0.0.0.0/0"
  tags              = { Name = "demo12b_rds-sgr-egress-all-2" }
}
