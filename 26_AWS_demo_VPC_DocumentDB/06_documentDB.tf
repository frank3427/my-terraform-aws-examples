...
resource "aws_vpc_security_group_ingress_rule" "demo26_docdb_ingress_all_0" {
  security_group_id = aws_security_group.demo26_docdb.id
  description       = "allow access from VPC"
  from_port         = var.docdb_port
  to_port           = var.docdb_port
  ip_protocol       = "tcp"
  cidr_ipv4         = var.cidr_vpc
  tags              = { Name = "demo26_docdb-sgr-ingress-all-0" }
}

resource "aws_vpc_security_group_egress_rule" "demo26_docdb_egress_all_1" {
  security_group_id = aws_security_group.demo26_docdb.id
  ...
  ip_protocol       = "-1"
  cidr_ipv4         = "0.0.0.0/0"
  tags              = { Name = "demo26_docdb-sgr-egress-all-1" }
}
